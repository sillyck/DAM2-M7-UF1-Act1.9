...
public class codificacionEnigma extends JFrame {
	...
	public class encript implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// cridar la funcio de comprovar caracters
			comprovarCaracters(chars);
	...
	public void comprovarCaracters(char[] array) {
		...
		if (entrada.getText().isEmpty()) {
			System.out.println("Error: El text d'entrada no pot estar buit");
			error = true;
		} else if(!error){
			for (int i = 0; i < array.length; i++) {
				...
				if (!matchFound && error == false) {
					System.out.println("Error: El text d'entrada han de ser lletres majuscules i no ha contindre numeros");
					error = true;
				}
				if (array[i] == ' ' && error == false) {
					System.out.println("Error: El text d'entrada no pot tindre espais en blanc");
					error = true;
				}
				if (array[i] == 'Ñ') {
					array[i] = 'N';
